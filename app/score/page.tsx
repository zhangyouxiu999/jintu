...
export function TableDemo({isSetAllSource}) {
  return (
    <div>
      <Table>
        ...
        <TableBody>
          {invoices.map((invoice) => (
            <TableRow key={invoice.id}>
              ...
              <TableCell className="text-center">
                {isSetAllSource ? <Input type="text" /> : <span>{invoice.score}</span>}
...
export default function Page() {
  const [isSetAllSource, setIsSetAllSource] = useState(false);

  function handleAdd() {
    setIsSetAllSource(!isSetAllSource);
  }

  return (
    <main>
      <h1>Add</h1>
      <Button className="cursor-pointer" onClick={handleAdd}>{isSetAllSource ? '取消' : '添加'}</Button>
      <Button className="cursor-pointer" >保存</Button>
      <TableDemo isSetAllSource={isSetAllSource} />
    </main>
  );
}